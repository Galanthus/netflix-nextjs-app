...
export function Title({
  ...
  const textColor =
    variant === "secondary" ? "text-green-400" : inverse ? "text-white" : "text-gray-800"
  return (
    <Tag
      className={clsx(
        "tracking-tight",
...
export function Subtitle({ children, variant = "blue", className }: SubtitleProps) {
  return (
    <span
      className={clsx(
...
export function Intro({ children, className }: React.PropsWithChildren<IntroProps>) {
  return <span className={clsx("text-2xl text-gray-600", className)}>{children}</span>
}
